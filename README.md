...
| the deployed instrument, re-certified after a provider swap | **F1 0.801 [0.752–0.844]** on the same gold set (2026-09-24) | DeepSeek re-pointed the requested model id at V4.1 Flash on 2026-09-10 and the within-run drift watch saw nothing; on the swapped model the certified request shape lost 24.9% of gold reviews to parse failures (F1 0.700), prompt-only JSON lost none. Production runs the re-certified shape; the misattribution and judge readings below were measured on the prior model and are pending re-measurement |
