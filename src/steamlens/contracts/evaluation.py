...
versions triple, the exact gold bytes (path + hash), the bootstrap dial
(seed, resamples), and the scorer's identity. The metric values themselves
...
@dataclass(frozen=True, slots=True)
class EvalRun:
    """One certification run: what was scored, against which gold, how — and the numbers.
    ...
    codebook bytes behind the triple's version label; ``gold_path`` +
    ``gold_sha256`` pin the measuring stick the same way, because gold is a
    living artifact (wording batches, the holdout) and a number scored against
    yesterday's gold must say so. ``n_scored_reviews`` may be smaller than
    ``n_gold_reviews`` when gold reaches outside the pool's scope (the CS2
    case: gold predates the usable-pool ruling) — the narrowing is stored, not
    buried in prose. ``scorer`` names the scoring procedure's identity so a
    future change in pairing semantics can never silently mix with old rows.
    """
